	...
	vector<Process> run(bool verbose, bool vverbose, vector<Process> process, printAll* p){
		...
		while(finished_proc!=n_proc){
			...
			if(verbose){
				...
				for(int i=0;i<n_proc;i++){
					...
					if(val>=0) cout<<"blocked"<<setw(10)<<blocked[val].ioT<<setw(10);
						...
						val = findIdx(ready,i);
						if(val>=0) cout<<"ready"<<setw(10)<<ready[val].burstT<<setw(10);
						else {
							val = findIdx(finished,i);
							if(val>=0) cout<<"terminated"<<setw(10)<<""<<setw(10);
							else {
								val = findIdx(waiting, i);
								if(val>=0) cout<<"unstarted"<<setw(10)<<waiting[val].burstT<<setw(10);
								else cout<<"running"<<setw(10)<<running[0].burstT<<setw(10);
							}
						}
					}
				}
			...
			if(!blocked.empty()){
				...
				if(blocked[0].ioT==0){
					if(vverbose) cout<<endl<<"Random Number for Burst Time:  ";
					blocked[0].randomBurstT(random);
					running.push_back(blocked[0]);
					blocked.clear();
				}
			} 
			if(running.size()>0){
